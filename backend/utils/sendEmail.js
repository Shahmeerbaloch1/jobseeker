...
export const sendResetPasswordEmail = async (email, code) => {
    try {
        const transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
                user: process.env.EMAIL_USER || 'balochshahmeer725@gmail.com',
                pass: process.env.APP_PASSWORD
            }
        })
        ...
        <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.1); border: 1px solid #e0e0e0;">
            ...
            <div style="padding: 40px 30px; background-color: #ffffff;">
                <h2 style="color: #1f2937; margin-top: 0; font-size: 20px; font-weight: 700; text-align: center; margin-bottom: 25px;">Reset Your Password</h2>
                
                <p style="color: #4b5563; font-size: 16px; line-height: 1.6; margin-bottom: 25px; text-align: center;">
                    We received a request to reset your password. Use the verification code below to proceed:
                </p>
                
                <div style="background-color: #fef2f2; border-radius: 12px; padding: 20px; text-align: center; margin: 30px 0; border: 1px dashed #f87171;">
                    <span style="font-family: 'Courier New', monospace; font-size: 32px; font-weight: 900; color: #dc2626; letter-spacing: 5px; display: block;">${code}</span>
                </div>
        ...
        const mailOptions = {
            from: `"Job Seeker" <${process.env.EMAIL_USER}>`,
            to: email,
            subject: 'Password Reset Verification - Job Seeker',
            html: htmlTemplate
        }
